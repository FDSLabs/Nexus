...
SIMAPP = ./app
HTTPS_GIT := https://github.com/FDSLabs/Nexus.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=nexus \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(nexus_BINARY) \
          -X github.com/cosmos/cosmos-sdk/version.Version=$(VERSION) \
          -X github.com/cosmos/cosmos-sdk/version.Commit=$(COMMIT) \
          -X "github.com/cosmos/cosmos-sdk/version.BuildTags=$(build_tags_comma_sep)" \
          -X github.com/cometbft/cometbft/version.TMCoreSemVer=$(TMVERSION) \
          -X github.com/FDSLabs/Nexus/version.AppVersion=$(VERSION) \
          -X github.com/FDSLabs/Nexus/version.GitCommit=$(COMMIT) \
          -X github.com/FDSLabs/Nexus/version.BuildDate=$(shell date -u +"%Y-%m-%dT%H:%M:%SZ")
...
build-docker:
	...
	$(DOCKER) create --name nexus -t -i ${DOCKER_IMAGE}:latest nexus
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp nexus:/usr/bin/nexusd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/nexus/nexus/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(nexus_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(nexus_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(nexus_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(nexus_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 50 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/nexus/nexus
.PHONY: format
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(nexus_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\nexus\Z nexusd/node "./nexusd testnet --v 4 -o /nexus --keyring-backend=test --ip-addresses nexusdnode0,nexusdnode1,nexusdnode2,nexusdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(nexus_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/nexus:Z nexusd/node "./nexusd testnet --v 4 -o /nexus --keyring-backend=test --ip-addresses nexusdnode0,nexusdnode1,nexusdnode2,nexusdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\nexusd:nexus\Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\nexusd:nexus\Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\nexusd:nexus\Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\nexusd:nexus\Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/nexusd:/nexus:Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/nexusd:/nexus:Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/nexusd:/nexus:Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/nexusd:/nexus:Z nexusd/node "./nexusd unsafe-reset-all --home=/nexus"
endif
...
.PHONY: build-docker-local-nexus localnet-start localnet-stop
