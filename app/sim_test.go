...
func TestFullAppSimulation(t *testing.T) {
	...
	appOptions[flags.FlagChainID] = "nexus_12345-1"

	// TODO: shadowed
	nexusApp := NewNexus(logger, db, nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(types.TestnetChainID+"-1"))
	require.Equal(t, nexusconfig.AppName, nexusApp.Name())
	...
	_, simParams, simErr := simulation.SimulateFromSeed(
		...
		os.Stdout,
		nexusApp.BaseApp,
		AppStateFn(nexusApp.AppCodec(), nexusApp.SimulationManager()),
		RandomAccounts, // replace with own random account function if using keys other than secp256k1
		simtestutil.SimulationOperations(nexusApp, nexusApp.AppCodec(), config),
		nexusApp.ModuleAccountAddrs(),
		config,
		nexusApp.AppCodec(),
	)

	// export state and simParams before the simulation error is checked
	err = simtestutil.CheckExportSimulation(nexusApp, config, simParams)
...
func TestAppImportExport(t *testing.T) {
	...
	app := NewNexus(logger, db, nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(types.TestnetChainID+"-1"))
	require.Equal(t, nexusconfig.AppName, app.Name())
	...
	newApp := NewNexus(log.NewNopLogger(), newDB, nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(types.TestnetChainID+"-1"))
	require.Equal(t, nexusconfig.AppName, newApp.Name())
...
func TestAppStateDeterminism(t *testing.T) {
	...
	appOptions.SetDefault(flags.FlagChainID, "nexus_12345-1")
