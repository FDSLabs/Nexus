...
import (
	...
	"github.com/FDSLabs/Nexus/cmd/config"
...
// Setup initializes a new nexus. A Nop logger is set in nexus.
func Setup(
	isCheckTx bool,
	feemarketGenesis *feemarkettypes.GenesisState,
) *Nexus {
	db := dbm.NewMemDB()
	app := NewNexus(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, false, simtestutil.NewAppOptionsWithFlagHome(DefaultNodeHome), baseapp.SetChainID(types.MainnetChainID+"-1"))
	if !isCheckTx {
...
func SetupWithGenesisAccounts(genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *Nexus {
...
func PrintModuleAddresses() {
	fmt.Printf("Module Addresses with nexus1 prefix:\n")
