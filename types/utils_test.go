...
func TestGetnexusAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"nexus address",
			"nexus1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5lrvs80",
			"nexus1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5lrvs80",
			false,
		},
		{
			"cosmos address",
			"cosmos1sm2nkjrv8lhq3u0fyfeer4vjpxxtcmnn6axk76",
			"nexus1sm2nkjrv8lhq3u0fyfeer4vjpxxtcmnn6uxclf",
			false,
		},
		{
			"osmosis address",
			"osmo1sm2nkjrv8lhq3u0fyfeer4vjpxxtcmnnjx4xgg",
			"nexus1sm2nkjrv8lhq3u0fyfeer4vjpxxtcmnn6uxclf",
			false,
		},
	}
	...
	for _, tc := range testCases {
		addr, err := GetnexusAddressFromBech32(tc.address)
		if tc.expError {
...
func TestGenerateAddresses(t *testing.T) {
	...
	nexusAddr := sdk.MustBech32ifyAddressBytes("nexus", address)
	...
	t.Logf("nexus address: %s", nexusAddr)
