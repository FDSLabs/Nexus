...
const (
	// MainnetChainID defines the nexus EIP155 chain ID for mainnet
	MainnetChainID = "nexus_1234"
	// TestnetChainID defines the nexus EIP155 chain ID for testnet
	TestnetChainID = "nexus_12345"
)

// IsMainnet returns true if the chain-id has the nexus mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the nexus testnet EIP155 chain prefix.
...
// GetnexusAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `nexus`).
// The function fails if the provided bech32 address is invalid.
func GetnexusAddressFromBech32(address string) (sdk.AccAddress, error) {
