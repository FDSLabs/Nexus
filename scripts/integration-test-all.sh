...
KEY="mykey"
CHAINID="nexus_12345-1"
MONIKER="mymoniker"

## default port prefixes for nexusd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
DATA_DIR=$(mktemp -d -t nexus-datadir.XXXXX)
...
init_func() {
    "$PWD"/build/nexusd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/nexusd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/nexusd add-genesis-account \
    "$("$PWD"/build/nexusd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000anexus,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/nexusd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/nexusd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/nexusd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    echo "starting nexus node $i in background ..."
    "$PWD"/build/nexusd start --pruning=nothing --rpc.unsafe \
    ...
    nexus_PID=$!
    echo "started nexus node, pid=$nexus_PID"
    # add PID to array
    arr+=("$nexus_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test nexus node $HOST_RPC ..."
...
stop_func() {
    nexus_PID=$i
    echo "shutting down node, pid=$nexus_PID ..."
