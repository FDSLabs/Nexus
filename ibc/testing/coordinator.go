...
// NewCoordinator initializes Coordinator with N EVM TestChain's (nexus apps) and M Cosmos chains (Simulation Apps)
func NewCoordinator(t *testing.T, nEVMChains, mCosmosChains int) *Coordinator {
	...
	// setup EVM chains
	DefaultTestingAppInit = NexusTestingAppInit

	for i := 1; i <= nEVMChains; i++ {
		chainID := GetChainIDNexus(i)
		chains[chainID] = NewTestChainNexus(t, coord, chainID)
	}
...
// GetChainID returns the chainID used for the provided index.
func GetChainIDNexus(index int) string {
	return ChainIDPrefixNexus + strconv.Itoa(index)
}
