...
import (
	...
	"github.com/FDSLabs/Nexus/ibc/testing/simapp"
	...
	nexusapp "github.com/FDSLabs/Nexus/app"
)

var DefaultTestingAppInit func() (TestingApp, map[string]json.RawMessage) = SetupTestingApp

var NexusTestingAppInit func() (TestingApp, map[string]json.RawMessage) = SetupTestingNexusApp
...
func SetupTestingNexusApp() (TestingApp, map[string]json.RawMessage) {
	db := dbm.NewMemDB()
	app := nexusapp.NewNexus(log.NewNopLogger(), db, nil, true, map[int64]bool{}, nexusapp.DefaultNodeHome, 5, false, simtestutil.EmptyAppOptions{})

	return app, nexusapp.NewDefaultGenesisState()
}
...
// of one consensus engine unit (10^6) in the default token of the simapp from first genesis
// account. A Nop logger is set in SimApp.
func SetupWithGenesisValSetNexus(tb testing.TB, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, balances ...banktypes.Balance) TestingApp {
	tb.Helper()
	app, genesisState := DefaultTestingAppInit()
	...
	_, err = app.InitChain(
		&abci.RequestInitChain{
			ChainId:         chainID,
			Validators:      []abci.ValidatorUpdate{},
			ConsensusParams: nexusapp.DefaultConsensusParams,
			AppStateBytes:   stateBytes,
		},
	)
	require.NoError(tb, err)
