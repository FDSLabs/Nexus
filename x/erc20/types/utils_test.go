	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "nexus",
				Symbol:      "nexus",
				Description: "EVM, staking and governance denom of nexus",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "anexus",
						Exponent: 0,
						Aliases:  []string{"atto nexus"},
					},
					{
						Denom:    "nexus",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			[]string{"atto nexus", "anexus"},
			[]string{"anexus", "atto nexus"},
			false,
		},
		{
			"same values, sorted",
