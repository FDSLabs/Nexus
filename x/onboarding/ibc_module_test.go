...
import (
	...
	ibctesting "github.com/FDSLabs/Nexus/ibc/testing"
...
func (suite *TransferTestSuite) SetupTest() {
	...
	suite.chainB = suite.coordinator.GetChain(ibctesting.GetChainIDNexus(1))
}
...
func (suite *TransferTestSuite) TestHandleMsgTransfer() {
	...
	coins := sdk.NewCoins(
		...
		sdk.NewCoin("anexus", sdkmath.NewInt(10000000000)),
	)
	err := suite.chainB.App.(*app.Nexus).BankKeeper.MintCoins(suite.chainB.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = suite.chainB.App.(*app.Nexus).BankKeeper.SendCoinsFromModuleToAccount(suite.chainB.GetContext(), inflationtypes.ModuleName, suite.chainB.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)

	// Setup params for coinswap
	coinswapKeeper := suite.chainB.App.(*app.Nexus).GetCoinswapKeeper()
	coinswapKeeper.SetStandardDenom(suite.chainB.GetContext(), "anexus")
	...
	middlewareParams := suite.chainB.App.(*app.Nexus).GetOnboardingKeeper().GetParams(suite.chainB.GetContext())
	middlewareParams.AutoSwapThreshold = sdkmath.NewInt(4000000)
	suite.chainB.App.(*app.Nexus).GetOnboardingKeeper().SetParams(suite.chainB.GetContext(), middlewareParams)

	erc20Keeper := suite.chainB.App.(*app.Nexus).GetErc20Keeper()
	...
	balanceVoucherBefore := suite.chainB.App.(*app.Nexus).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), voucherDenomTrace.IBCDenom())
	balanceNexusBefore := suite.chainB.App.(*app.Nexus).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), "anexus")
	...
	balanceVoucher := suite.chainB.App.(*app.Nexus).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), voucherDenomTrace.IBCDenom())
	balanceNexus := suite.chainB.App.(*app.Nexus).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), "anexus")
	...
	// check whether the nexus is swapped and the amount is greater than the threshold
	if balanceNexusBefore.Amount.LT(middlewareParams.AutoSwapThreshold) {
		suite.Require().Equal(balanceNexus.Amount, balanceNexusBefore.Amount.Add(middlewareParams.AutoSwapThreshold))
	} else {
		suite.Require().Equal(balanceNexus.Amount, balanceNexusBefore.Amount)
	}
	...
	blockedAddr := "nexus1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5wgmgwp" // gov module
	...
	suite.Require().NoError(err)
	suite.Require().Equal([]byte(`{"error":"ABCI code: 1: error handling packet: see events for details"}`), ack)
	...
	suite.Require().Equal(balanceNexusBefore, balanceNexus)
	suite.Require().Equal(balanceVoucherBefore, balanceVoucher)
