...
import (
	...
	ibcgotesting "github.com/FDSLabs/Nexus/ibc/testing"
...
type IBCTestingSuite struct {
	...
	nexusChain      *ibcgotesting.TestChain
	IBCGravityChain *ibcgotesting.TestChain
	IBCCosmosChain  *ibcgotesting.TestChain

	pathGravitynexus  *ibcgotesting.Path
	pathCosmosnexus   *ibcgotesting.Path
...
func (suite *IBCTestingSuite) SetupTest() {
	...
	suite.nexusChain = suite.coordinator.GetChain(ibcgotesting.GetChainIDNexus(1))
	suite.IBCGravityChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(2))
	suite.IBCCosmosChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(3))
	suite.coordinator.CommitNBlocks(suite.nexusChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCGravityChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCCosmosChain, 2)

	// Mint coins on the gravity side which we'll use to unlock our anexus
	...
	// Mint coins on the cosmos side which we'll use to unlock our anexus
	...
	suite.nexusChain.App.(*app.Nexus).OnboardingKeeper.SetParams(suite.nexusChain.GetContext(), params)

	// Setup the paths between the chains
	suite.pathGravitynexus = ibcgotesting.NewTransferPath(suite.IBCGravityChain, suite.nexusChain) // clientID, connectionID, channelID empty
	suite.pathCosmosnexus = ibcgotesting.NewTransferPath(suite.IBCCosmosChain, suite.nexusChain)
	suite.pathGravityCosmos = ibcgotesting.NewTransferPath(suite.IBCCosmosChain, suite.IBCGravityChain)
	suite.coordinator.Setup(suite.pathGravitynexus) // clientID, connectionID, channelID filled
	suite.coordinator.Setup(suite.pathCosmosnexus)
	suite.coordinator.Setup(suite.pathGravityCosmos)
	suite.Require().Equal("07-tendermint-0", suite.pathGravitynexus.EndpointA.ClientID)
	suite.Require().Equal("connection-0", suite.pathGravitynexus.EndpointA.ConnectionID)
	suite.Require().Equal("channel-0", suite.pathGravitynexus.EndpointA.ChannelID)
	...
	suite.nexusChain.CurrentHeader.ProposerAddress = suite.nexusChain.LastHeader.ValidatorSet.Proposer.Address
...
// FundNexusChain mints coins and sends them to the nexusChain sender account
func (suite *IBCTestingSuite) FundNexusChain(coins sdk.Coins) {
	err := suite.nexusChain.App.(*app.Nexus).BankKeeper.MintCoins(suite.nexusChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = suite.nexusChain.App.(*app.Nexus).BankKeeper.SendCoinsFromModuleToAccount(suite.nexusChain.GetContext(), inflationtypes.ModuleName, suite.nexusChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
}
...
func (suite *IBCTestingSuite) setupRegisterCoin(metadata banktypes.Metadata) *erc20types.TokenPair {
	err := suite.nexusChain.App.(*app.Nexus).BankKeeper.MintCoins(suite.nexusChain.GetContext(), inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
	suite.Require().NoError(err)

	pair, err := suite.nexusChain.App.(*app.Nexus).Erc20Keeper.RegisterCoin(suite.nexusChain.GetContext(), metadata)
	suite.Require().NoError(err)
	return pair
}

// CreatePool creates a pool with anexus and the given denom
func (suite *IBCTestingSuite) CreatePool(denom string) {
	coinnexus := sdk.NewCoin("anexus", sdkmath.NewIntWithDecimal(10000, 18))
	coinIBC := sdk.NewCoin(denom, sdkmath.NewIntWithDecimal(10000, 6))
	coins := sdk.NewCoins(coinnexus, coinIBC)
	suite.FundNexusChain(coins)

	coinswapKeeper := suite.nexusChain.App.(*app.Nexus).CoinswapKeeper
	coinswapKeeper.SetStandardDenom(suite.nexusChain.GetContext(), "anexus")
	coinswapParams := coinswapKeeper.GetParams(suite.nexusChain.GetContext())
	coinswapParams.MaxSwapAmount = sdk.NewCoins(sdk.NewCoin(denom, sdkmath.NewIntWithDecimal(10, 6)))
	coinswapKeeper.SetParams(suite.nexusChain.GetContext(), coinswapParams)
	...
	msgAddLiquidity := coinswaptypes.MsgAddLiquidity{
		...
		Sender:           suite.nexusChain.SenderAccount.GetAddress().String(),
	}

	// Add liquidity to the pool
	suite.nexusChain.App.(*app.Nexus).CoinswapKeeper.AddLiquidity(suite.nexusChain.GetContext(), &msgAddLiquidity)
}
